...
def movePoint(name):
    points = name.split(':')
    intpoints = []
    for point in points[0:len(points)]:
        intpoints.append(int(point))
    robotMove(80, intpoints)

def help():
    print("Available commands: RUN, HOME, GOTO, DEFINE")
...
while 1:
    ...
    if name == 'GOTO':
        while 1:
            name = input('Go To Point: ')
            if name == 'EXIT': break
            moveFilePoint(name)
    if name == 'DEFINE':
        while 1:
            name = input('Enter Point (x:x:x:x:x:x:x): ')
            if name == 'EXIT': break
            movePoint(name)
            name = input('Name point (Type EXIT to discard): ')
            if name == 'EXIT': break
